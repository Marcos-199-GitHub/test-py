...
horario = pd.read_csv('archivos/horarios25_2.csv', encoding='utf-8')  # Why latin-1?
...
def obtener_datos():
    ...
    # a algún valor
    seleccion = horario[horario['Profesor'] == 'CASTAÑEDA GALVAN ADRIAN ANTONIO']
    ...
    eRooms = np.sort(horario.loc[vac, 'Salon'].unique()).tolist()
    # print(eRooms)

    # En otro caso, colocar un salón y buscar que horarios está ocupado
    salon = horario[horario['Salon'] == '318']
    ...
    return (horaActual, horaCompar, diaSemActual, profesores, seleccion.to_dict(orient='records'),
            horario[colSem[int(diaSemActual)]].to_string(), eRooms, salon.to_dict(orient='list'))
...
def getHorarioProfesor(profesor):
    ...
    """
    seleccion = horario[horario['Profesor'] == profesor]
    return seleccion.to_dict(orient='records')


def getSalonesLibresYOcupados(hora_cmp, dia_sem):
    """
    Función que obtiene los salones libres en la hora actual
    :param hora_cmp: str; Hora en formato HH:MM
    :param dia_sem: int; Día de la semana 0-5 (Lunes-Sabado)
    :return: list; Salones vacíos en la hora actual y list; Salones ocupados en la hora actual
    """
    horaCompar = dt.datetime.strptime(hora_cmp, '%H:%M').time()
    diaSemActual = dia_sem

    colSem = horario.columns[5:11].tolist()
    vac = []
    ocupados = []
    for j, dd in enumerate(horario[colSem[diaSemActual]]):
        # print(dd)
        if dd == '0':  # Si no hay horario
            ...
            try:
                ...
                ini = dt.datetime.strptime(ini, '%H:%M').time()
                fin = dt.datetime.strptime(fin, '%H:%M').time()
                # print(ini, horaCompar, fin, ini <= horaCompar < fin)
                if ini <= horaCompar < fin:
                    ocupados.append(j)
                else:
                    vac.append(j)
    ...
    salones_libres = np.sort(horario.loc[vac, 'Salon'].unique()).tolist()
    salones_ocupados = np.sort(horario.loc[ocupados, 'Salon'].unique()).tolist()
    return [i for i in salones_libres if i not in salones_ocupados], salones_ocupados
...
def getHorarioSalon(salon):
    ...
    """
    seleccion = horario[horario['Salon'] == salon]
    return seleccion.to_dict(orient='records')
