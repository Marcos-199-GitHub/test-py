...
from polls.utils.acciones import *
import json
...
def profesores_view(request):
    ...
    :return:
        response : JsonResponse; Lista de nombres de profesores
    """
...
@csrf_exempt
def salones_alumnos_view(request):
    hora_cmp = json.loads(request.body).get('hora')
    dia_sem = json.loads(request.body).get('dia_semana')
    response = JsonResponse({'result': getSalonesLibresYOcupados(hora_cmp, dia_sem)[2]})
    return response
...
@csrf_exempt
def ocupar_salon_view(request):
    salon = json.loads(request.body).get('salon')
    horario = json.loads(request.body).get('horario')
    response = JsonResponse({'result': ocuparSalon(salon, horario)})
    return response
